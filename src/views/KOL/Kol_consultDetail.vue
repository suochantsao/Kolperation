    ...
    <div class="consultDetArea">
      <ul class="caseBasic">
        ...
        <ul class="conditionBlock">
          ...
          <li class="companyName" @click="linkToFirm">
            {{ caseDetail.Company }}
          </li>
          ...
          <kol-channel-item :channels="channelList"></kol-channel-item>
        </ul>

        <btn-add-fav :contentId="caseId" :boolStr="favBool"> </btn-add-fav>
      </ul>

      <ul class="caseInfo">
        <ul class="caseDetail">
          <li class="blockTitle">合作內容</li>
          <p>{{ caseDetail.Detail }}<br /></p>
          <ul class="btnBlock">
            <a>
              <li
                class="applyBtn"
            ...
            <a>
              <li class="askBtn" @click="contactFirm">
    ...
    applyCase() {
      ...
      this.$http
        ...
        .then((res) => {
          ...
          this.$router.push({
            path: `/kolplat/sucessDetail?msg=${this.caseId}`,
          });
        })
        .catch((err) => {
          console.error(err);
